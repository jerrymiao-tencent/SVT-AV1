...
static INLINE __m512i loadu_8bit_32x2_avx512(const void *const src,
    const uint32_t strideInByte) {
    const __m256i src0 = _mm256_loadu_si256((__m256i *)src);
    const __m256i src1 =
        _mm256_loadu_si256((__m256i *)((uint8_t *)src + strideInByte));
    return _mm512_setr_m256i(src0, src1);
}

static INLINE __m512i loadu_u8_32x2_avx512(const uint8_t *const src,
    const uint32_t stride) {
    return loadu_8bit_32x2_avx512(src, sizeof(*src) * stride);
}

static INLINE __m512i loadu_s16_16x2_avx512(const int16_t *const src,
    const uint32_t stride) {
    return loadu_8bit_32x2_avx512(src, sizeof(*src) * stride);
}

static INLINE void storeu_8bit_32x2_avx512(const __m512i src, void *const dst,
    const int32_t strideInByte) {
...
static INLINE void prepare_half_coeffs_2tap_avx512(
...
static INLINE void prepare_half_coeffs_4tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    ...
    coeffs[0] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0604u));
    // coeffs 4 5 4 5 4 5 4 5
    coeffs[1] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0a08u));
}

static INLINE void prepare_half_coeffs_6tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    ...
    coeffs[0] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0402u));
    // coeffs 3 4 3 4 3 4 3 4
    coeffs[1] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0806u));
    // coeffs 5 6 5 6 5 6 5 6
    coeffs[2] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0C0Au));
}

SIMD_INLINE void prepare_half_coeffs_8tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    ...
    coeffs[0] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0200u));
    // coeffs 2 3 2 3 2 3 2 3
    coeffs[1] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0604u));
    // coeffs 4 5 4 5 4 5 4 5
    coeffs[2] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0a08u));
    // coeffs 6 7 6 7 6 7 6 7
    coeffs[3] = _mm512_shuffle_epi8(coeffs_1, _mm512_set1_epi16(0x0e0cu));
}

static INLINE void prepare_coeffs_2tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    __m512i *const coeffs /* [1] */) {
...
static INLINE void prepare_coeffs_4tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    __m512i *const coeffs /* [2] */) {
    const int16_t *filter = av1_get_interp_filter_subpel_kernel(
        *filter_params, subpel_q4 & SUBPEL_MASK);
...
static INLINE void prepare_coeffs_6tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    __m512i *const coeffs /* [3] */) {
    const int16_t *const filter = av1_get_interp_filter_subpel_kernel(
        *filter_params, subpel_q4 & SUBPEL_MASK);
...
static INLINE void prepare_coeffs_8tap_avx512(
    const InterpFilterParams *const filter_params, const int32_t subpel_q4,
    __m512i *const coeffs /* [4] */) {
    const int16_t *filter = av1_get_interp_filter_subpel_kernel(
        *filter_params, subpel_q4 & SUBPEL_MASK);
...
static INLINE void load_16bit_32x7_avx512(const int16_t *const src,
    __m512i dst[7]) {
...
static INLINE void convolve_8tap_unapck_avx512(const __m512i s[6],
    __m512i ss[7]) {
...
static INLINE __m512i convolve_2tap_avx512(const __m512i ss[1],
    const __m512i coeffs[1]) {
    return _mm512_maddubs_epi16(ss[0], coeffs[0]);
}

static INLINE __m512i convolve_4tap_avx512(const __m512i ss[2],
...
static INLINE __m512i convolve_6tap_avx512(const __m512i ss[3],
...
static INLINE __m512i convolve_8tap_avx512(const __m512i ss[4],
...
static INLINE __m512i convolve16_2tap_avx512(const __m512i ss[1],
    const __m512i coeffs[1]) {
    return _mm512_madd_epi16(ss[0], coeffs[0]);
}

static INLINE __m512i convolve16_4tap_avx512(const __m512i ss[2],
    const __m512i coeffs[2]) {
...
static INLINE __m512i convolve16_6tap_avx512(const __m512i ss[3],
    const __m512i coeffs[3]) {
...
static INLINE __m512i convolve16_8tap_avx512(const __m512i ss[4],
    const __m512i coeffs[4]) {
    const __m512i res_01 = _mm512_madd_epi16(ss[0], coeffs[0]);
    const __m512i res_23 = _mm512_madd_epi16(ss[1], coeffs[1]);
    const __m512i res_45 = _mm512_madd_epi16(ss[2], coeffs[2]);
...
static INLINE __m512i jnt_avg_round_avx512(const __m512i src,
    const __m512i offset) {
    const __m512i dst = _mm512_add_epi16(src, offset);
    return _mm512_srai_epi16(dst, 2);
}

static INLINE __m512i jnt_no_avg_round_avx512(const __m512i src,
    const __m512i offset) {
    const __m512i dst = _mm512_add_epi16(src, offset);
...
static INLINE void xy_y_pack_store_32x2_avx512(const __m512i res0,
    const __m512i res1,
    uint8_t *const dst,
    const int32_t stride) {
    const __m512i t = _mm512_packus_epi16(res0, res1);
    const __m512i d =
        _mm512_permutexvar_epi64(_mm512_setr_epi64(0, 4, 2, 6, 1, 5, 3, 7), t);
    storeu_u8_32x2_avx512(d, dst, stride);
}

static INLINE void xy_y_pack_store_64_avx512(const __m512i res0,
    const __m512i res1,
    uint8_t *const dst) {
...
static INLINE void xy_y_round_store_32x2_avx512(const __m512i r0[2],
    const __m512i r1[2],
    uint8_t *const dst,
    const int32_t stride) {
...
static INLINE void xy_y_round_store_64_avx512(const __m512i r0[2],
    const __m512i r1[2],
    uint8_t *const dst) {
    const __m512i ra = xy_y_round_32_avx512(r0);
    const __m512i rb = xy_y_round_32_avx512(r1);
...
static INLINE void convolve_store_32x2_avx512(const __m512i res0,
    const __m512i res1,
    uint8_t *const dst,
...
static INLINE void convolve_store_64_avx512(const __m512i res0,
    const __m512i res1,
    uint8_t *const dst) {
    const __m512i d = _mm512_packus_epi16(res0, res1);
    _mm512_storeu_si512((__m512i *)dst, d);
...
static INLINE void jnt_no_avg_store_32x2_avx512(const __m512i src0,
    const __m512i src1,
...
static INLINE void jnt_no_avg_store_64_avx512(const __m512i src0,
    const __m512i src1,
    ConvBufType *const dst) {
    const __m512i d0 = _mm512_permutex2var_epi64(
        src0, _mm512_setr_epi64(0, 1, 8, 9, 2, 3, 10, 11), src1);
...
static INLINE void x_convolve_2tap_32x2_avx512(const uint8_t *const src,
...
static INLINE void x_convolve_2tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[1],
    __m512i r[2]) {
...
static INLINE __m512i x_convolve_6tap_avx512(const __m512i data,
    const __m512i coeffs[3],
...
static INLINE __m512i x_convolve_8tap_avx512(const __m512i data,
    const __m512i coeffs[4],
...
static INLINE void x_convolve_6tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[3],
    const __m512i filt[3],
    __m512i r[2]) {
...
static INLINE void x_convolve_6tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[3],
    const __m512i filt[3],
    __m512i r[2]) {
...
SIMD_INLINE void x_convolve_8tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[4],
    const __m512i filt[4],
    __m512i r[2]) {
...
SIMD_INLINE void x_convolve_8tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[4],
    const __m512i filt[4],
    __m512i r[2]) {
    const __m512i s0_512 = _mm512_loadu_si512((__m512i *)src);
...
static INLINE void y_convolve_2tap_32x2_avx512(const uint8_t *const src,
    const int32_t stride,
    const __m512i coeffs[1],
    __m256i *const s0_256,
    __m512i r[2]) {
...
static INLINE void y_convolve_2tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[1],
    const __m512i s0,
...
static INLINE void y_convolve_4tap_32x2_avx512(
    const uint8_t *const src, const int32_t stride, const __m512i coeffs[2],
    __m256i s_256[3], __m512i ss_512[4], __m512i r[2]) {
...
static INLINE void y_convolve_6tap_32x2_avx512(
    const uint8_t *const src, const int32_t stride, const __m512i coeffs[3],
    __m256i s_256[5], __m512i ss_512[6], __m512i r[2]) {
...
static INLINE void y_convolve_6tap_64x2_avx512(
    const uint8_t *const src, const int32_t stride, const __m512i coeffs[3],
    __m512i s_512[6], __m512i ss_512[6], __m512i tt_512[6], __m512i r[4]) {
...
static INLINE void y_convolve_8tap_32x2_avx512(
    const uint8_t *const src, const int32_t stride, const __m512i coeffs[4],
    __m256i s_256[7], __m512i ss_512[8], __m512i r[2]) {
...
static INLINE void y_convolve_8tap_64x2_avx512(
    const uint8_t *const src, const int32_t stride, const __m512i coeffs[4],
...
static INLINE void xy_x_convolve_2tap_32x2_avx512(const uint8_t *const src,
    const int32_t stride,
    const __m512i coeffs[1],
    __m512i r[2]) {
...
static INLINE void xy_x_convolve_2tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[1],
    __m512i r[2]) {
    const __m512i s0 = _mm512_loadu_si512((__m512i *)src);
    const __m512i s1 = _mm512_loadu_si512((__m512i *)(src + 1));
...
static INLINE void xy_x_2tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[1],
    int16_t *const dst) {
    __m512i r[2];
...
static INLINE void xy_x_6tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[3],
    const __m512i filt[3],
    int16_t *const dst) {
...
static INLINE void xy_x_6tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[3],
    const __m512i filt[3],
    int16_t *const dst) {
    __m512i d[2];
...
static INLINE void xy_x_8tap_32x2_avx512(const uint8_t *const src,
    const int32_t src_stride,
    const __m512i coeffs[4],
    const __m512i filt[4],
    int16_t *const dst) {
...
static INLINE void xy_x_8tap_64_avx512(const uint8_t *const src,
    const __m512i coeffs[4],
    const __m512i filt[4],
    int16_t *const dst) {
    __m512i d[2];
...
static INLINE void xy_y_convolve_2tap_32_avx512(const __m512i s0,
    const __m512i s1,
    const __m512i coeffs[1],
    __m512i r[2]) {
...
static INLINE void xy_y_convolve_2tap_32x2_avx512(const int16_t *const src,
    __m512i s[2],
    const __m512i coeffs[1],
    __m512i r[4]) {
...
static INLINE void xy_y_convolve_2tap_64_avx512(const int16_t *const src,
    const __m512i s0[2],
    __m512i s1[2],
    const __m512i coeffs[1],
    __m512i r[4]) {
...
static INLINE void xy_y_convolve_2tap_64_all_avx512(const int16_t *const src,
    const __m512i s0[2],
    __m512i s1[2],
    const __m512i coeffs[1],
    uint8_t *const dst) {
...
static INLINE void xy_y_convolve_2tap_half_pel_32x2_avx512(
    const int16_t *const src, __m512i s[2], __m512i r[2]) {
...
static INLINE void xy_y_convolve_2tap_half_pel_64_all_avx512(
    const int16_t *const src, const __m512i s0[2], __m512i s1[2],
    uint8_t *const dst) {
    __m512i r[2];
...
SIMD_INLINE void xy_y_convolve_6tap_width32x2_avx512(
    const int16_t *const src, const __m512i coeffs[3], __m512i s_512[6],
...
static INLINE void xy_y_convolve_6tap_32x2_avx512(
    const int16_t *const src, const int32_t stride, __m512i s_512[6],
...
SIMD_INLINE void xy_y_convolve_8tap_32_avx512(const __m512i *const ss,
    const __m512i coeffs[4],
    __m512i r[2]) {
...
SIMD_INLINE void xy_y_convolve_8tap_width32x2_avx512(
    const int16_t *const src, const __m512i coeffs[4], __m512i s_512[8],
...
static INLINE __m512i jnt_comp_avg_convolve_32_avx512(const __m512i res,
    const __m512i dst,
...
static INLINE void jnt_comp_avg_round_store_32_kernel_avx512(
...
static INLINE void jnt_loadu_u16_8x4x2_avx512(const ConvBufType *const src,
    const int32_t stride,
    __m512i d[2]) {
    const __m512i s0 = _mm512_loadu_si512((__m512i *)(src + 0 * stride));
    const __m512i s1 = _mm512_loadu_si512((__m512i *)(src + 1 * stride));
    d[0] = _mm512_permutex2var_epi64(
        s0, _mm512_setr_epi64(0, 1, 4, 5, 8, 9, 12, 13), s1);
    d[1] = _mm512_permutex2var_epi64(
        s0, _mm512_setr_epi64(2, 3, 6, 7, 10, 11, 14, 15), s1);
}
...
SIMD_INLINE void jnt_comp_avg_round_store_64_avx512(
    const __m512i res[2], const __m512i factor, const __m512i offset,
    const ConvBufType *const dst, uint8_t *const dst8) {
    __m512i r[2], d[2];
...
static INLINE void jnt_copy_comp_avg_32_avx512(
    const uint8_t *const src, const __m512i factor_512,
    const __m512i offset_comp_avg_512, const ConvBufType *const dst,
    uint8_t *const dst8) {
    const __m512i res = jnt_copy_load_src_32_avx512(src);
    jnt_comp_avg_round_store_32_kernel_avx512(
        res, factor_512, offset_comp_avg_512, dst, dst8);
}

static INLINE void jnt_avg_round_store_32x2_avx512(
...
static INLINE void jnt_avg_round_store_64_avx512(const __m512i res[2],
    const __m512i offset,
...
static INLINE void jnt_no_avg_round_store_32x2_avx512(
...
static INLINE void jnt_no_avg_round_store_64_avx512(const __m512i res[2],
    const __m512i offset,
    ConvBufType *const dst) {
    __m512i d[2];
